...
export default function Payment() {
  ...
  const validateCard = () => {
    ...
    if (digits.length < 13) {
      e.cardNumber = 'رقم البطاقة غير صحيح'
    } else if (!luhnCheck(cardForm.cardNumber)) {
      e.cardNumber = 'رقم البطاقة غير صالح'
    }
    if (!validateExpiry(cardForm.expiry)) {
      e.expiry = 'تاريخ انتهاء الصلاحية غير صحيح'
    }
    if (!validateCVV(cardForm.cvv)) {
      e.cvv = 'رمز التحقق غير صحيح'
    }
    if (!cardForm.cardholderName.trim()) {
      e.cardholderName = 'يرجى إدخال اسم حامل البطاقة'
    }
    setCardErrors(e)
  ...
  if (orderComplete) {
    return (
      <div className="max-w-md mx-auto px-4 py-16 text-center">
        ...
        <h1 className="text-2xl font-bold text-gray-800 mb-2">تم تقديم الطلب بنجاح!</h1>
        <p className="text-gray-600 mb-6">
          سنتواصل معك على {order.shipping.phone} لتأكيد التوصيل
        </p>
        <Link
          to="/"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-3">
            ...
            >
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">ادفع نقداً أو ببطاقة عند استلام الطلب</p>
              </div>
            </label>
            <label
              className={`flex items-center gap-4 p-4 border-2 rounded-xl cursor-pointer transition-colors ${
        ...
        <div>
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 sticky top-24">
            <h2 className="font-semibold text-gray-800 mb-4">ملخص الطلب</h2>
