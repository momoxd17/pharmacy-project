...
export default function SearchBar({ className = '', onClose, variant = 'desktop' }: SearchBarProps) {
    ...
    <>
      <form onSubmit={handleSubmit} className={`relative flex items-center ${className}`}>
        ...
        <input
          ...
          placeholder={'search for medicines, vitamins, care...'}
      ...
      {cameraOpen && (
        <div className="fixed inset-0 bg-black/90 z-50 flex flex-col items-center justify-center p-4">
          ...
          <p className="text-white mb-4">التقط صورة للمنتج للبحث عنه</p>
          ...
          >
            {ocrLoading ? ('analyzing') : ('searchByImage')}
          </button>
        </div>
      )}
    </>
