...
import { LayoutDashboard, Package, FolderTree, ArrowRight } from 'lucide-react'
...
export default function AdminLayout() {
  ...
  const navItems = [
    ...
    { to: '/admin/products', icon: Package, labelKey: 'products' as const, end: false },
    { to: '/admin/categories', icon: FolderTree, labelKey: 'categories' as const, end: false },
  ]
        ...
        <nav className="flex-1 p-4 space-y-1">
          {navItems.map(({ to, icon: Icon, labelKey, end }) => (
            ...
            >
              ...
              {t(labelKey as 'dashboard' | 'products' | 'categories')}
            </NavLink>
          ))}
        </nav>
